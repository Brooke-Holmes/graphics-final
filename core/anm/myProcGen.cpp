...
anm::MeshData anm::createNoisePlane(float width, float height, int subdivisions, bool perSegment, ew::Vec2* points)
{
	anm::MeshData plane;
	...
	for (int row = 0; row <= subdivisions; row++) {
		for (int col = 0; col <= subdivisions; col++) {
			...
			float xDist = pos.x - points[0].x;
			float yDist = pos.y - points[0].y;
			float distance = sqrt((pow(xDist, 2.0f) + pow(yDist, 2.0f)));
			for (int j = 1; j < sizeof(points); j++)
			{
				xDist = pos.x - points[j].x;
				yDist = pos.y - points[j].y;
...
ew::MeshData anm::createPlane(float width, float height, int subdivisions, bool perSegment)
{
	ew::MeshData plane;
	if (subdivisions <= 0) { subdivisions = 1; }
